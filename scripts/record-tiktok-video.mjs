// Deterministic 600-frame capture of the med-spa TikTok promo (20s, 1080x1920, 30fps).
// Same recipe as fitness/tattoo: 1080x1920 surface + CDP metrics override (540x960 CSS at
// scale 2, mobile), raw CDP screenshots, and a VIRTUAL rAF clock so the three.js silk-flow,
// counters, and marquee animate at output-timeline speed instead of capture speed.
// Med-spa extra: at t=4s the script clicks the "Acne & scarring" finder chip so the
// treatment finder — the signature feature — demos itself on camera.
//
// Usage (from a folder with `npm i playwright`; needs Google Chrome + ffmpeg installed):
//   1. Serve this framework:  python3 -m http.server 5630 --directory <med-spa>
...
//        -vf format=yuv420p -movflags +faststart -an med-spa-founding-offer-20s.mp4
...
await page.goto("http://localhost:5630/", { waitUntil: "load" });
...
// Webfonts (Prata) must be in before any frame is captured.
await page.evaluate(() => document.fonts.ready);

// Force-load everything (lazy images, spa strip bg) with a pre-scroll.
await page.waitForFunction(() => document.querySelectorAll("#treatmentsGrid img").length > 0, { timeout: 15000 });
...
await page.waitForFunction(() => {
  const imgs = [...document.querySelectorAll("#treatmentsGrid img, #teamGrid img")];
...
await page.evaluate(() => {
  // Virtual rAF clock: the silk-flow render loop advances exactly with the output timeline.
  let vt = 0;
  ...
  `;
  cap.textContent = "Built for medspas & skin clinics";
  ...
  card.innerHTML = `
    ...
    <div style="font-size:27px;color:#cfc9c0;line-height:1.45;margin-bottom:44px;">A website like this,<br>for <span style="font-style:italic;color:#d9a04c;">your</span> clinic.</div>
  ...
  const yOf = sel => sel === 0 ? 0 : Math.max(0, document.querySelector(sel).getBoundingClientRect().top + window.scrollY - headerH - 6);
  // Selectors, not pixel values: the chip click at CHIP_AT inserts ~1000px of
  // finder cards, so targets below the finder must be re-measured per frame.
  const segments = [
    [1.8, 3.4, 0, "#finder"],
    [6.4, 8.0, "#finder", "#treatments"],
    [9.8, 11.4, "#treatments", "#memberships"],
    [12.6, 14.6, "#memberships", "#consult"],
  ];
  const CHIP_AT = 4.0;            // click a finder chip while parked on the finder
  const CAP_IN = 6.8, CAP_OUT = 11.0, CARD_IN = 15.7, FADE = 0.55;
  ...
  window.__pose = async (t) => {
    ...
    if (!chipDone && t >= CHIP_AT) { chipDone = true; window.setConcern("acne"); }

    let y = 0;
    for (const [t0, t1, s0, s1] of segments) {
      const y0 = yOf(s0), y1 = yOf(s1);
    ...
    if (marquee) marquee.style.transform = `translateX(-${((t / 30) * 50) % 50}%)`;
